...
///////////////////////////////////////////////////////////////////////////////
		...
		$this->dbHost     = "sql311.eshost.es";      //The host to connect to.
		$this->dbUser     = "eshos_12426320";       //The username to login as.
		$this->dbPassword = "Seazoux";         //The database password.
		$this->dbDatabase = "eshos_12426320_smf";     //The database to utilize.
...
///////////////////////////////////////////////////////////////////////////////
	...
	function showStars($ratableKey) {
		...
		$textDesc = "<div style=\"float:left;\" id=\"rabidRating-$ratingId-description\" class=\"ratingText\">"
					.$this->getStarMessage($rating)."</div>";

		$textDesc2 = "<div style=\"float:right;width:32px;text-align:right;\" id=\"rabidRating-$ratingId-description2\" class=\"ratingText\">"
					.$this->getStarMessage2($rating)."</div>";

		/* Here's where you would mess with the code to modify the template. */
		echo "<div style=\"height: 25px;\">".$textDesc.$textDesc2."<div style=\"float:right;\"><div id=\"rabidRating-$ratingId-$stars"."_$this->stars\" "
			 ."class=\"rabidRating$unratedClass\"><div class=\"wrapper\">"
			 ."<span class=\"ratingFill\" style=\"width:$percent;\">"
			 ."<span class=\"ratingStars\"></span>"
			 ."</span></div></div></div></div>";
	}

	function showStars2($ratableKey) {
		$rating = $this->loadRating($ratableKey);
		$ratingId = $rating['keyID'];
		$stars = $this->percentToStars($rating['rating']);
		$percent = round($rating['rating'], 0)."%";
	...
	function getStarMessage($rating) {
		...
		return "$rating[totalRatings] vote$s";
	}

	function getStarMessage2($rating) {
		$stars = $this->percentToStars($rating['rating']);
